...
void Shape::init()
{
   ...
   glBindVertexArray(vaoID);
    ...
    for (int i = 0; i < eleBuf.size(); i += 3) {
        v0ndx = eleBuf[i] * 3;
        v0 = vec3(posBuf[v0ndx], posBuf[v0ndx + 1], posBuf[v0ndx + 2]);

        v1ndx = eleBuf[i + 1] * 3;
        v1 = vec3(posBuf[v1ndx], posBuf[v1ndx + 1], posBuf[v1ndx + 2]);

        v2ndx = eleBuf[i + 2] * 3;
        v2 = vec3(posBuf[v2ndx], posBuf[v2ndx + 1], posBuf[v2ndx + 2]);

        u = v1 - v0;
        w = v2 - v0;
        faceNormal = cross(u, w);

        norBuf[v0ndx] += faceNormal.x;
        norBuf[v1ndx] += faceNormal.x;
        norBuf[v2ndx] += faceNormal.x;

        norBuf[v0ndx + 1] += faceNormal.y;
        norBuf[v1ndx + 1] += faceNormal.y;
        norBuf[v2ndx + 1] += faceNormal.y;

        norBuf[v0ndx + 2] += faceNormal.z;
        norBuf[v1ndx + 2] += faceNormal.z;
        norBuf[v2ndx + 2] += faceNormal.z;
    } 

    vec3 temp;
    for (int i = 0; i < norBuf.size(); i += 3) {
        temp = normalize(vec3(norBuf[i], norBuf[i + 1], norBuf[i + 2])); 
        norBuf[i] = temp.x;
        norBuf[i + 1] = temp.y;
        norBuf[i + 2] = temp.z;
    }
